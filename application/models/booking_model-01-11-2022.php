...
class Booking_model extends CI_Model {
	...
	function GetReportBooking($party_id,$brand_id,$category_id,$booking_date_from,$booking_date_to,$booked_by='',$status='',$perPage=20, $pageNo=1,$employee=''){ 
    ...
    function CountBooking($party_id,$brand_id,$category_id,$booking_date_from,$booking_date_to,$booked_by='',$status='',$employee=''){ 
		//echo date('Y-m-d');  die;
	...
	function GetBookingSummary($party_id,$brand_id,$category_id,$booking_date_from,$booking_date_to,$booked_by='',$status='',$group_by,$employee=''){   
		$condition = "";
		$cur_date = date('Y-m-d');
		if($booking_date_from!='' && $booking_date_to=='')
			$condition = "`booking_booking.created_at` BETWEEN '".$booking_date_from." 00:00:00.000000' AND '".$cur_date." 23:54:54.969999'"; 
		...
		$this->db->select(array('count(booking_booking.id) as bargain_count','booking_booking.shipment_date','booking_booking.production_unit','booking_booking.is_lock','booking_booking.remaining_weight','booking_booking.is_close','SUM(booking_booking.total_weight) as weight', 'SUM(booking_booking.quantity) as quantity','booking_booking.booking_id','booking_booking.id','booking_booking.party_id','booking_booking.brand_id','booking_booking.category_id','AVG(booking_booking.rate) as avg_rate','booking_booking.created_at','booking_booking.admin_id','vendors.name as party_name','vendors.city_id as city_id','city.name as city_name','brands.name as brand_name','category.category_name as category_name','brokers.name as broker_name','admin.name as admin_name'));
		$this->db->select('booking_booking.status');
		$this->db->select('booking_booking.remark');
	    $this->db->from('booking_booking'); 
	    $this->db->join('vendors', 'vendors.id = booking_booking.party_id','left');  
	...
	function GetBookingSummarySumReport($party_id,$brand_id,$category_id,$booking_date_from,$booking_date_to,$booked_by='',$status='',$group_by,$employee=''){   
		$condition = "";
		$cur_date = date('Y-m-d');
		if($booking_date_from!='' && $booking_date_to=='')
			$condition = "`booking_booking.created_at` BETWEEN '".$booking_date_from." 00:00:00.000000' AND '".$cur_date." 23:54:54.969999'"; 
	...
	function GetBookingSummaryLocked($party_id,$brand_id,$category_id,$booking_date_from,$booking_date_to,$booked_by='',$status='',$group_by,$employee=''){   
		$condition = "";
		$cur_date = date('Y-m-d');
		if($booking_date_from!='' && $booking_date_to=='')
			$condition = "`booking_booking.created_at` BETWEEN '".$booking_date_from." 00:00:00.000000' AND '".$cur_date." 23:54:54.969999'"; 
