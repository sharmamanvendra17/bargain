...
class Booking_model extends CI_Model {
	...
	function getdispatchlist($part_id,$product_id)
	{
		$data_query ="SELECT SUM(`quantity`) as quantity,`pi_number`,pi_history.dispatch_date, STR_TO_DATE(pi_history.dispatch_date, '%d-%m-%Y') as d1, DATEDIFF(now(), STR_TO_DATE(pi_history.dispatch_date, '%d-%m-%Y')) as days FROM `pi_sku_history` JOIN pi_history on pi_history.id= pi_sku_history.`pi_number` where pi_sku_history.party_id = $part_id and pi_sku_history.product_id = $product_id GROUP by STR_TO_DATE(pi_history.dispatch_date, '%d-%m-%Y') order by STR_TO_DATE(pi_history.dispatch_date, '%d-%m-%Y') DESC"; 
		//echo $data_query; die;
	...
	function GetReportBooking($party_id,$brand_id,$category_id,$booking_date_from,$booking_date_to,$booked_by='',$status='',$perPage=20, $pageNo=1,$employee='',$unit='',$pending='',$bagainnumber='',$broker=''){ 
		...
		{
			$condition = "booking_booking.status <> 6";
		}
		...
		$this->db->select(array('b1.loose_oil_rate_kg_max','booking_booking.loose_oil_rate_kg','booking_booking.dispatch_status',  'booking_booking.insurance_amount', 'pih.total_weight_pi', 'pih.pi_ids','booking_booking.reject_remark','admin.unauthorized_viewers','booking_booking.loose_oil_rate','booking_booking.is_for','booking_booking.is_mail','booking_booking.shipment_date','booking_booking.production_unit','booking_booking.is_lock','booking_booking.remaining_weight','booking_booking.is_close','booking_booking.total_weight as weight', 'booking_booking.quantity','booking_booking.booking_id','booking_booking.id','booking_booking.party_id','booking_booking.brand_id','booking_booking.category_id','booking_booking.rate','booking_booking.created_at','booking_booking.admin_id','vendors.name as party_name','vendors.city_id as city_id','city.name as city_name','brands.name as brand_name','category.category_name as category_name','brokers.name as broker_name','admin.name as admin_name','admin.mobile as admin_mobile','admin.team_lead_id','vendors.state_id'));

		 
		$this->db->select("pih.withouttruck");
		$this->db->select("pih.withtruck");
		$this->db->select("bs.not_pi_sku");
		
		$this->db->select("dispatch.total_dispatch as total_dispatch");
	    ...
	    $this->db->join("(SELECT booking_id,count(booking_skus.id) as not_pi_sku from booking_skus where pi_id = 0 group by booking_id) as bs", 'bs.booking_id = booking_booking.id ','left');
 
 		$this->db->join("(SELECT pi_sku_history.booking_id,SUM(pi_sku_history.weight) as total_dispatch from pi_sku_history join pi_history  on pi_history.id = pi_sku_history.pi_number  where pi_history.dispatch_date IS NOT NULL AND pi_history.dispatch_date <> '' group by pi_sku_history.booking_id) as dispatch", 'dispatch.booking_id = booking_booking.id ','left');


 		$this->db->join("( select b3.loose_oil_rate_kg as loose_oil_rate_kg_max, b3.category_id from  booking_booking as b3 join ( SELECT MAX(booking_booking.id) as b_id, booking_booking.category_id from booking_booking group by booking_booking.category_id) b2 on b2.b_id= b3.id ) as b1", 'b1.category_id = booking_booking.category_id and booking_booking.status = 2','left');

	    
	    //$this->db->join("(SELECT GROUP_CONCAT(`pi_history`.`id`) as pi_ids, SUM(`pi_history`.`total_weight_pi`) as total_weight_pi, pi_history.booking_id,if(pi_history.truck_number IS NULL OR pi_history.truck_number ='' , count(pi_history.id), 0) as withouttruck,if(pi_history.truck_number IS NOT NULL, count(pi_history.id), 0) as withtruck FROM `pi_history` WHERE pi_history.status=0 group by `booking_id`) as pih", ' FIND_IN_SET ( booking_booking.id , pih.booking_id ) ' ,'LEFT',false);

	    $this->db->join("(SELECT GROUP_CONCAT(t.pi_ids) as pi_ids, sum(t.total_weight_pi) as total_weight_pi, t.booking_id,sum(t.withouttruck) as withouttruck, sum(t.withtruck) as withtruck from (SELECT pi_history.id as pi_ids,`booking_skus`.`id`,`booking_skus`.`pi_id`, SUM(`pi_history`.`total_weight_pi`) as total_weight_pi, booking_skus.booking_id,if(pi_history.truck_number IS NULL OR pi_history.truck_number ='' , count(pi_history.id), 0) as withouttruck,if(pi_history.truck_number IS NOT NULL, count(pi_history.id), 0) as withtruck FROM `pi_history` JOIN booking_skus ON `pi_history`.`id` = booking_skus.pi_id WHERE pi_history.status=0 group by booking_skus.`booking_id`, booking_skus.`pi_id`) as t group by t.`booking_id`) as pih", ' FIND_IN_SET ( booking_booking.id , pih.booking_id ) ' ,'LEFT',false);
		...
		if($bagainnumber)
	    	$this->db->where('booking_booking.booking_id',$bagainnumber);
		//if($booked_by)
			//$this->db->where('booking.admin_id',$booked_by);
        ...
        { 
            //$this->db->where('admin_id' , $userid);  
            //$this->db->where("(admin.team_lead_id = $userid OR admin_id = $userid)"); 
            $this->db->where("(admin.team_lead_id = $userid OR admin_id = $userid OR ( admin_id  = ( SELECT id from admin  where find_in_set ( $userid, admin.persona_user) )))");  
        }
        elseif ($role==2) {  //checker
            $this->db->where("(admin.team_lead_id = $userid OR admin_id = $userid)");  
        ...
        if($status!='')
        {
        	...
        	elseif($status=='pending_dispatch')
        	{
            	//$this->db->where("booking_booking.dispatch_status <>",1);
            	//$this->db->where("withouttruck <>",0);
            	$this->db->where("((booking_booking.status = 2 AND booking_booking.is_lock=0) OR (booking_booking.status <> 3 AND booking_booking.is_lock=2) OR ( booking_booking.is_lock=1 AND  withouttruck > 0))");
        	}
        	elseif($status=='partial_lock')
        	{
            	$this->db->where("booking_booking.is_lock ",2);
    	...
    	if($broker!='')
    	{ 
    		$this->db->where("booking_booking.broker_id",$broker); 
    	}
        ...
        if($pending)
        {
        	...
        	$this->db->where("DATE(booking_booking.created_at) <= DATE_SUB(CURDATE(), INTERVAL ".$pending_days." DAY)"); 
        	$this->db->where("booking_booking.dispatch_status <>",1);
        }
        $this->db->limit($perPage, $startFromRecord);
		$this->db->order_by('booking_booking.id','DESC');  
	...
	function getlast_booking_id($book_chek_date){  
		...
		$select_query =  " SELECT  MAX(booking_id) as booking_id FROM booking_booking"; 
    ...
    function CountBooking($party_id,$brand_id,$category_id,$booking_date_from,$booking_date_to,$booked_by='',$status='',$employee='',$unit='',$pending='',$bagainnumber='',$broker=''){ 
		//echo date('Y-m-d');  die;
	...
	function GetBookingSummary($party_id,$brand_id,$category_id,$booking_date_from,$booking_date_to,$booked_by='',$status='',$group_by,$employee='',$unit='',$pending='',$broker='',$bagainnumber=''){   
		$condition = "";
		$cur_date = date('Y-m-d');
		if($pending=='')
		{
		...
		$this->db->select("AVG( if( `vendors`.`state_id` <> 4 AND  `vendors`.`state_id` <> 22 AND  `vendors`.`state_id` <> 23 AND  `vendors`.`state_id` <> 24 AND  `vendors`.`state_id` <> 25 AND  `vendors`.`state_id` <> 30 AND  `vendors`.`state_id` <> 33 AND  `vendors`.`state_id` <> 3, `booking_booking`.`rate`, booking_booking.avg_condition )) as avg_rate_other");
		$this->db->select("AVG( if( `vendors`.`state_id` = 4 OR  `vendors`.`state_id` = 22 OR  `vendors`.`state_id` = 23 OR  `vendors`.`state_id` = 24 OR  `vendors`.`state_id` = 25 OR  `vendors`.`state_id` = 30 OR  `vendors`.`state_id` = 33 OR  `vendors`.`state_id` = 3 , `booking_booking`.`rate`, booking_booking.avg_condition )) as avg_rate_aasam");
		...
		$this->db->select("SUM( if( `vendors`.`state_id` <> 4 AND  `vendors`.`state_id` <> 22 AND  `vendors`.`state_id` <> 23 AND  `vendors`.`state_id` <> 24 AND  `vendors`.`state_id` <> 25 AND  `vendors`.`state_id` <> 30 AND  `vendors`.`state_id` <> 33 AND  `vendors`.`state_id` <> 3, `booking_booking`.`loose_oil_rate`*`booking_booking`.`quantity`, booking_booking.avg_condition ))/SUM( if( `vendors`.`state_id` <> 4 AND  `vendors`.`state_id` <> 22 AND  `vendors`.`state_id` <> 23 AND  `vendors`.`state_id` <> 24 AND  `vendors`.`state_id` <> 25 AND  `vendors`.`state_id` <> 30 AND  `vendors`.`state_id` <> 33 AND  `vendors`.`state_id` <> 3, `booking_booking`.`quantity`, booking_booking.avg_condition )) as avg_rate_other_loose");
		...
		$this->db->select("SUM( if( `vendors`.`state_id` = 4 OR  `vendors`.`state_id` = 22 OR  `vendors`.`state_id` = 23 OR  `vendors`.`state_id` = 24 OR  `vendors`.`state_id` = 25 OR  `vendors`.`state_id` = 30 OR  `vendors`.`state_id` = 33 OR  `vendors`.`state_id` = 3 , `booking_booking`.`loose_oil_rate`*`booking_booking`.`quantity`, booking_booking.avg_condition ))/SUM( if( `vendors`.`state_id` = 4 OR  `vendors`.`state_id` = 22 OR  `vendors`.`state_id` = 23 OR  `vendors`.`state_id` = 24 OR  `vendors`.`state_id` = 25 OR  `vendors`.`state_id` = 30 OR  `vendors`.`state_id` = 33 OR  `vendors`.`state_id` = 3 , `booking_booking`.`quantity`, booking_booking.avg_condition )) as avg_rate_aasam_loose");

		$this->db->select('SUM(skus.bulk_quantity) as bulk_quantity');
		$this->db->select('SUM(skus.bulk_weight) as bulk_weight');
		$this->db->select('SUM(skus.consumer_quantity) as consumer_quantity');
		$this->db->select('SUM(skus.consumer_weight) as consumer_weight');
	    ...
	    $this->db->join('admin', 'admin.id = booking_booking.admin_id','left');
 		...
 		$this->db->join("(SELECT booking_skus.booking_id, sum(if(`products`.`product_type` = 0,booking_skus.quantity,0))  as bulk_quantity, sum(if(`products`.`product_type` = 0,booking_skus.weight,0))  as bulk_weight, sum(if(`products`.`product_type` = 1,booking_skus.quantity,0))   as consumer_quantity, sum(if(`products`.`product_type` = 1,booking_skus.weight,0))  as consumer_weight, SUM(booking_skus.weight) as weight FROM `booking_skus` LEFT JOIN `products` ON `products`.`id` = `booking_skus`.`product_id`  GROUP BY booking_skus.booking_id) skus", "skus.booking_id = booking_booking.id" , "LEFT"); 
		...
		if($group_by)
		{
			...
			foreach ($group_by as $key => $group_by_value) { 
				if($group_by_value=='broker')
				{
					 $s =1;
					$this->db->group_by('booking_booking.broker_id');  
					$this->db->having("booking_booking.broker_id <> '' OR booking_booking.broker_id IS NOT NULL  OR booking_booking.broker_id <> 0");  
					$this->db->order_by('brokers.name','ASC'); 
				}
				elseif($group_by_value=='place')
				{
	...
	function GetBookingSummarySumReport($party_id,$brand_id,$category_id,$booking_date_from,$booking_date_to,$booked_by='',$status='',$group_by,$employee='',$rejected='',$unit='',$pending='',$broker='',$bagainnumber=''){   
		$condition = "";
		$cur_date = date('Y-m-d');
		if($pending=='')
		{
	...
	function GetBookingSummaryLocked($party_id,$brand_id,$category_id,$booking_date_from,$booking_date_to,$booked_by='',$status='',$group_by,$employee='',$unit='',$pending='',$broker='',$bagainnumber=''){   
		$condition = "";
		$cur_date = date('Y-m-d');
		if($pending=='')
		{
	...
	{	
		$this->db->select('booking_booking.insurance as booking_insurance');
		...
		$this->db->select('products.fixed_rate_flag');
		$this->db->select('products.fixed_rate');
		...
		$this->db->select('p.weight as base_weight');
		$this->db->select('empty_tin_rates.product_id as p_id');
        ...
        $this->db->join('products p','p.id= e1.product_id','LEFT');
	...
	function product_packing_type_summary($party_id,$brand_id,$category_id,$booking_date_from,$booking_date_to,$booked_by='',$status='',$group_by,$employee='',$rejected='',$unit='',$pending='',$broker='',$bagainnumber='')
	{
		$condition = "";
		$cur_date = date('Y-m-d');
		if($pending=='')
	...
	function check_sales_history_stock_1($party_id,$stock_date)
	{
		$data_query = "SELECT id FROM `cnf_sales_history` WHERE cnf_sales_history.stock_date like '%$stock_date%' and cnf_sales_history.party_id = $party_id";
		$query = $this->db->query($data_query); 
	    return $query->num_rows();
	}

	function check_sales_history_stock($party_id)
	{
		$data_query = "SELECT id FROM `cnf_sales_history` WHERE YEAR( cnf_sales_history.stock_date) = YEAR(CURRENT_DATE - INTERVAL 1 MONTH) AND MONTH( cnf_sales_history.stock_date) = MONTH(CURRENT_DATE - INTERVAL 1 MONTH) and cnf_sales_history.party_id = $party_id";
		$query = $this->db->query($data_query); 
	    return $query->num_rows();
	}
...
WHERE YEAR( booking_skus.created_at) = YEAR(CURRENT_DATE - INTERVAL 1 MONTH) AND MONTH( booking_skus.created_at) = MONTH(CURRENT_DATE - INTERVAL 1 MONTH) and  booking_booking.party_id = $party_id and booking_skus.pi_id != 0 GROUP by  booking_skus.product_id;";
	...
	function past_month_buy_amount($party_id,$month_date)
	{
		...
		$data_query = "SELECT brands.name as brand_name,category.category_name as category_name,products.name as name, products.packing_items_qty, pi_sku_history.party_id,pi_sku_history.brand_id,pi_sku_history.category_id,pi_sku_history.product_id,pi_sku_history.created_at,sum(pi_sku_history.quantity) as purchased_qty ,sum(pi_sku_history.weight) as purchased_weight ,sum(pi_sku_history.amount) as purchased_amount FROM pi_sku_history left join brands on brands.id =pi_sku_history.brand_id left join category on category.id =pi_sku_history.category_id left join products on products.id =pi_sku_history.product_id  WHERE pi_sku_history.created_at like '%$month_date%' and  pi_sku_history.party_id = $party_id and pi_sku_history.status = 0 GROUP by  pi_sku_history.category_id"; 
		$query = $this->db->query($data_query);  
	    return  $query->result_array(); 
	}
	...
	function checkflags($condition)
	{
		$this->db->select(array('pih.total_weight_pi', 'pih.pi_ids','booking_booking.is_lock','booking_booking.remaining_weight','booking_booking.booking_id','booking_booking.id'));
	...
	function GetDispetchqtyreport($type,$month,$state,$production_unit)
	{

		//$this->db->select('*');		 
		$this->db->select('SUM(pi_sku_history.quantity) as total_cartons');	
		$this->db->select('SUM(pi_sku_history.quantity*products.packing_items_qty) as total_qty');		 
		$this->db->select("products.name as product_name");   
		//$this->db->select("booking_booking.production_unit");   
	    ...
	    $this->db->where("products.name like '%".$type."%'");  
	    $this->db->where("(pi_history.truck_number IS NOT NULL OR pi_history.truck_number <> '')"); 
	    $this->db->where("pi_history.status", 0);  
	    $this->db->where("(pi_history.dispatch_date LIKE '%".$month."%')");

	    if($state)
	    {
	    	$this->db->join("vendors", 'vendors.id = pi_sku_history.party_id','left');	  
	    	$this->db->where("vendors.state_id",$state);
	    }

	    if($production_unit)
	    {
	    	//$this->db->join('booking_booking', 'FIND_IN_SET ( booking_booking.id , pi_history.booking_id )' ,'LEFT');
	    	$this->db->join('booking_booking', 'booking_booking.id = pi_sku_history.booking_id' ,'LEFT');
	    	$this->db->where("booking_booking.production_unit",$production_unit);
	    }
	    ...
	    //echo $this->db->last_query(); die;
	    $row = array();
	    $data = array('total_qty' => 0,'name' => $type);
	    if($query->num_rows() > 0 )
	    {
	      $row = $query->row_array();  
	      $data = array('total_qty' => $row['total_qty'],'name' => $type,'total_cartons' => $row['total_cartons'],);
	    }
	    return $data;  
	}


	function GetDispetchqtydetailreport($type,$month,$state,$production_unit)
	{

		//$this->db->select('*');		 
		$this->db->select('SUM(pi_sku_history.weight) as weight');	
		$this->db->select('SUM(pi_sku_history.quantity) as total_cartons');	
		$this->db->select('SUM(pi_sku_history.quantity*products.packing_items_qty) as total_qty');		 
		$this->db->select("products.name as product_name");   
		//$this->db->select("booking_booking.production_unit");   
		...
		$this->db->group_by("products.packaging_type"); 
	    $this->db->group_by("products.weight"); 
	...
	function whatsappusers($booking_ids)
	{
		$this->db->select('brokers.mobile as brokers_mobile');
		$this->db->select('vendors.mobile as vendor_mobile');
		$this->db->select('admin.mobile as asm_mobile');

		$this->db->select('brokers.name as brokers_name');
		$this->db->select('vendors.name as vendor_name');
		$this->db->select('admin.name as asm_name');

        $this->db->from('booking_booking');
        $this->db->join("vendors", 'vendors.id = booking_booking.party_id','left');	 
        $this->db->join("brokers", 'brokers.id = booking_booking.broker_id','left');	  
        $this->db->join("admin", 'admin.id = booking_booking.admin_id','left');	  
        $this->db->where("booking_booking.id IN ($booking_ids)"); 
        $query = $this->db->get();
		//echo $this->db->last_query(); die;  
        $response = array();
        if($query->num_rows())
        {
        	$results =  $query->result_array();
        	foreach ($results as $key => $value) { 
        		if($value['brokers_mobile'])
        		{ 
        			$response[$value['brokers_mobile']] = $value['brokers_name'];
        		}
        		if($value['vendor_mobile'])
        		{
        			$response[$value['vendor_mobile']] = $value['vendor_name'];
        		}
        		if($value['asm_mobile'])
        			$response[$value['asm_mobile']] = $value['asm_name'];
	...
	function BargainDispatchDates($booking_id)
	{
		//$this->db->select("GROUP_CONCAT(DISTINCT(STR_TO_DATE(`dispatch_date`,'%d,%m,%Y'))) as dispatch_date",false); 
		$this->db->select('GROUP_CONCAT(pi_history.dispatch_date) as dispatch_date'); 
		$this->db->from('pi_history');  
        $this->db->where("find_in_set ($booking_id, pi_history.booking_id )");  
        $query = $this->db->get();   
        //echo $this->db->last_query(); die;  
	...
	function GetBargainerId($condition)
	{ 		
		$this->db->select('booking_booking.admin_id'); 
		$this->db->from('booking_booking');   
		$this->db->where($condition);  
		$query = $this->db->get();   
		return $query->row_array();
	...
	function GetBargainername($condition)
	{
		$this->db->select('admin.name as bargainer_name'); 
		$this->db->from('admin');   
		$this->db->where($condition);  
		$query = $this->db->get();   
		//echo $this->db->last_query(); die;  
	...
	function GetBargainapproved_whatsapp_status($booking_id)
	{
		$this->db->select('booking_booking.approved_whatsapp_status'); 
		$this->db->from('booking_booking');   
		$this->db->where('booking_booking.booking_id',$booking_id);
		$query = $this->db->get();   
		//echo $this->db->last_query(); die;  
		...
		if($query->num_rows())
        {
        	...
        	$res = $result['approved_whatsapp_status'];
        }
		return $res;
	}

	function GetBookingDispatch($party_id,$brand_id,$category_id,$booking_date_from,$booking_date_to,$booked_by='',$status='',$perPage=20, $pageNo=1,$employee='',$unit='',$pending='',$bagainnumber='',$broker='')
	{
		...
		$this->db->select('SUM(pi_sku_history.weight)  as total_dispateched_weight'); 
		$this->db->select('SUM(pi_sku_history.amount)  as total_dispatched_amount'); 
		$this->db->from('pi_history');   
		$this->db->join('pi_sku_history', 'pi_history.id = pi_sku_history.pi_number','left');  
		$this->db->join('booking_booking', 'booking_booking.id = pi_sku_history.booking_id','left');  
		$this->db->join('vendors', 'vendors.id = booking_booking.party_id','left');  
		$this->db->join('brands', 'brands.id = booking_booking.brand_id','left');  
		$this->db->join('category', 'category.id = booking_booking.category_id','left');  
        ...
        $this->db->where("pi_history.dispatch_date IS NOT NULL"); 
        $this->db->where("pi_history.dispatch_date <> ", ""); 
        $this->db->where("booking_booking.status <> ", 3); 
        $this->db->where("pi_history.status ", 0);  
        $query = $this->db->get();  
        return $query->row_array(); 
	...
	function GetBookingBargains($party_id,$brand_id,$category_id,$booking_date_from,$booking_date_to,$booked_by='',$status='',$perPage=20, $pageNo=1,$employee='',$unit='',$pending='',$bagainnumber='',$broker='')
	{
		...
		$this->db->select('SUM(booking_booking.total_weight)  as total_bargain_weight');  
		$this->db->from('booking_booking'); 
	    $this->db->join('vendors', 'vendors.id = booking_booking.party_id','left');  
	    $this->db->join('city', 'city.id = vendors.city_id','left');  
	    $this->db->join('brands', 'brands.id = booking_booking.brand_id','left');  
        ...
        $this->db->where("(booking_booking.status = 2 OR booking_booking.status = 6)");  
        $query = $this->db->get();   
       	return $query->row_array(); 
		//echo $this->db->last_query(); die;  
	} 
	function GetBookingSummaryStateCity($condition){
	    ...
	    if($query->num_rows())	
	    {
	    	...
	    	foreach ($row as $key => $value) {
			    ...
			    $this->db->join('states', 'states.id = vendors.state_id','left'); 
			    $this->db->join('city', 'city.id = vendors.city_id','left'); 
			    $this->db->join('brands', 'brands.id = pi_sku_history.brand_id','left'); 
			    $this->db->join('category', 'category.id = pi_sku_history.category_id','left'); 
			    if($condition)
			   		$this->db->where($condition);   
			   	$this->db->where('pi_sku_history.party_id',$value['party_id']);   
			   	$this->db->group_by('pi_sku_history.party_id');   
			   	$this->db->group_by('pi_sku_history.brand_id');   
			   	$this->db->group_by('pi_sku_history.category_id');   
			   	$this->db->order_by('pi_sku_history.party_id','ASC');
			   	$query2 = $this->db->get();   
			   	if($query2->num_rows())	
	    		{
	    			$detailed_res = $query2->result_array();
	    			//echo "<pre>"; print_r($detailed_res); die;
	    		}
	    		$row[$key]['detailed_res'] =  $detailed_res; 
	    	}
	    }
	...
	function GetBookingSummaryStateCitywithoutparty($condition){
	    ...
	    $this->db->from('pi_sku_history');   
	    $this->db->join('pi_history', 'pi_history.id = pi_sku_history.pi_number','left');
	    $this->db->join('vendors', 'vendors.id = pi_sku_history.party_id','RIGHT'); 
	    $this->db->join('states', 'states.id = vendors.state_id','left'); 
	    $this->db->join('city', 'city.id = vendors.city_id','left'); 
	    $this->db->join('brands', 'brands.id = pi_sku_history.brand_id','left'); 
	    $this->db->join('category', 'category.id = pi_sku_history.category_id','left'); 
	...
	function GetDispatchedSummaryBrandProduct($condition){
		$this->db->select('sum(pi_sku_history.weight) as total_dispatch');
		//$this->db->select('pi_sku_history.party_id'); 
		//$this->db->select('vendors.name as party_name'); 
		$this->db->select('category.category_name'); 
	...
	function GetBaraginSummaryBrandProduct($booking_date_from,$booking_date_to){
		$condition = "";
		if($booking_date_from!='' && $booking_date_to=='')
			$condition = "`booking_booking.created_at` BETWEEN '".$booking_date_from." 00:00:00.000000' AND '".$cur_date." 23:54:54.969999'"; 
		elseif($booking_date_from!='' && $booking_date_to!='')
			$condition = "`booking_booking.created_at` BETWEEN '".$booking_date_from." 00:00:00.000000' AND '".$booking_date_to." 23:54:54.969999'";
		$this->db->select('SUM(booking_booking.total_weight) as weight'); 
		$this->db->select('category.category_name'); 
		$this->db->select('brands.name as brand_name');  
	    $this->db->from('booking_booking');    
	    $this->db->join('brands', 'brands.id = booking_booking.brand_id','left'); 
	    ...
	    if($condition)
	   	...
	   	$this->db->group_by('booking_booking.brand_id');     
	    $query = $this->db->get();  
	    $row = array();
	    //echo $this->db->last_query(); die;  
	    if($query->num_rows())	
	...
	function getallcompnieswithlastdispatch($condition){

	 
		$this->db->select('vendors.name as party_name');  
		$this->db->select('vendors.mobile as party_mobile'); 
		$this->db->select('MAX(STR_TO_DATE(pi_history.dispatch_date,"%d-%m-%Y")) as dispatch_date', false);  
		$this->db->select('states.name as state_name'); 
		$this->db->select('city.name as city_name'); 
	    $this->db->from('vendors');   
	    //$this->db->join('pi_history', 'pi_history.id = pi_sku_history.pi_number','left'); ; 
	    $this->db->join('states', 'states.id = vendors.state_id','left'); 
	    $this->db->join('pi_history', 'pi_history.party_id = vendors.id','left'); ; 
	    $this->db->join('city', 'city.id = vendors.city_id','left'); 
	    $this->db->where_not_in('vendors.id',$condition['parties']);   
	    unset($condition['parties']); 
	    if($condition)
	   		$this->db->where($condition);   
	   	$this->db->where('vendors.status',1);   
	   	$this->db->group_by('vendors.id');    
	   	$this->db->order_by('dispatch_date','DESC');   
	    $query = $this->db->get(); 
	    //echo $this->db->last_query(); die;
	    $row = array();
	    if($query->num_rows())	
	...
	function AddWhatsappLog($insertdata)
	{
		return $this->db->insert('whatsapp_messages ',$insertdata);
		//echo $this->db->last_query(); die;
	}
}
