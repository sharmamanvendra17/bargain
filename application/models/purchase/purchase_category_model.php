...
class Purchase_category_model extends CI_Model {
	...
	function UpdateCategory($update_data,$condition) 
	{
  	...
  	function GetProductsbycategpry_id($condition){ 
		$this->db->select(array('pur_products.attributes','pur_products.id','pur_products.product_name','pur_category.category_name as category_name'));
	...
	function Getallattributesbyproduct_id($condition){
		$this->db->select('pur_product_attributes.*');
	    $this->db->from('pur_products');  
	    //$this->db->join("pur_product_attributes" ,"FIND_IN_SET(pur_product_attributes.id,pur_products.attributes)","LEFT",false);  
	    $this->db->join('pur_product_attributes', 'FIND_IN_SET ( pur_product_attributes.id , pur_products.attributes )' ,'LEFT');
	    $this->db->where($condition);  
	    $query = $this->db->get(); 
	    //echo $this->db->last_query(); die;
	    $row = $query->result_array(); 
	...
	function Getattributesbyproduct_id($attributes){
		$this->db->select('*');
	    $this->db->from('pur_product_attributes ');  
	    $this->db->where("id IN ($attributes)");  
	    $query = $this->db->get(); 
	    //echo $this->db->last_query(); die;
	    $row = $query->result_array(); 
	    return $row;  
