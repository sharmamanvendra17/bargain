                    ...
                    <table class="table table-striped table-bordered table-hover" id="">
                        ...
                        <tbody class="booking_records"> 
                                <?php $total_weight = 0; if($bookings) { 
                                    ...
                                    foreach ($bookings as $key => $value) { ?>
                                        <tr class="odd gradeX" style="text-align: center; vertical-align: middle;">
                                            ...
                                            ?> 
                                                <?php if($value['total_pi']) { ?>
                                                <span class="btn btn-danger pi_history"  data-status="3" rel="<?php echo base64_encode($value['id']); ?>" <?php echo ($value['is_lock']==0) ? 'style="cursor:none"' : '';?> > <img style="height: 17px;width: 17px;" src="<?php echo base_url('assets/img').$lock_img; ?>"><?php  echo ($value['status']==6) ? '<img style="height: 35px" src="'.base_url("assets/img/full-truck.png").'">' : "PI"; ?></span>
                                                <?php } else {  
                                                    if($value['status']==3)  
                                                    { ?>
                                                    ...
                                                    <span class="btn btn-danger <?php echo ($logged_role == 4) ? '' : 'style="cursor:none"'; ?>"  data-status="3" rel="<?php echo base64_encode($value['id']); ?>" > <img style="height: 17px;width: 17px;" src="<?php echo base_url('assets/img').$lock_img; ?>"> <?php echo ($value['status']==6) ? '<img style="height: 35px" src="'.base_url("assets/img/full-truck.png").'">' : "Approval Pending"; ?></span>
                                                    <?php } ?> 
                                                <?php } ?>
                                            </td> 
                                            <td>
                                            <?php
                                            if($value['status']!=6 ) {
                                            if($value['is_lock'] && $value['status']!=3 )
                                            { ?>
                                                <?php if($value['pending_sku_pi']) { ?>
                                                <input class="pi_select" type="checkbox" name="bargains[]" value="<?php echo $value['id']; ?>" data-weight="<?php echo round($value['weight'],2); ?>">
                                                <?php } elseif($value['is_lock']==1 && $value['pending_sku_pi']==0) { ?>
                                                     <span class="" >Pi Generated</span>
                                                <?php } else { ?>
                                                    <span class="" >No Pending SKU</span>
                                                <?php } ?>
                                            <?php } else { ?> 
                                ...
                                <?php if($bookings) { ?><tr>
                                    ...
                                    <td><button style="display:none;" type="submit" class="btn btn-default company_selection" value="Save">Generate PI</button></td>
                                </tr> 
                                <?php } ?>
                        </tbody>
                    </table>
...
<script>
    $(document).ready(function(){
        ...
        $("body").on("click",".whatsapp",function(e){ 
            e.preventDefault();
            var booking_ids = $(this).attr('rel');
            var pi_id = $(this).attr('data-pi');
            var bargain_ids = $(this).attr('data-bargain_ids');
            $("#divLoading").css({display: "block"});
            $.ajax({
                type: 'POST',
                url: '<?php echo base_url(); ?>performainvoice/whatsappusers',
                data: {'booking_ids' :booking_ids,'pi_id' :pi_id,'bargain_ids' :bargain_ids},
        ...
        $("body").on("click",".whatsapppdf",function(e){ 
            ...
            e.preventDefault();
            var numbers = Array();
            var pi_id = $("#notify_pi_id").val(); 
            var bargain_ids = $("#notify_bargain_ids").val(); 
            ...
            $.ajax({
                type: 'POST',
                url: '<?php echo base_url(); ?>performainvoice/whatsapppdf',
                data: {'numbers' : numbers,'pi_id' : pi_id,'bargain_ids' :bargain_ids},
                success: function(msg){ 
                    $("#divLoading").css({display: "none"});
                   // $("#DetailModal").modal('hide'); 
                    alert("Dispatched notification sent successfully");
                }
            });
        });

        $(document).on("submit", "#pi_deviation_modal_form", function(event){ 
            event.preventDefault(); 
            $(".loader").show();
            $.ajax({
                type: "POST",
                url: '<?php echo base_url();?>performainvoice/save_deviation/',
                data: $("#pi_deviation_modal_form").serializeArray(), 
                dataType: "html",
                success: function(data){
                    $(".loader").hide();  
                    if(data)
                    {
                        $(".DispacthSuccess_message").html("<span style='color:green'>Dispatch Updated Successfully.</span>");                        
                    }
                    else
                    {
                        $(".DispacthSuccess_message").html("<span style='color:red'>Please try again.</span>");   
                    }
                    $("#DispacthSuccess").modal('show');
                }
            });     
        });

        $(document).on("submit", "#pi_history_modal_form", function(event){ 
            event.preventDefault(); 
            $(".loader").show();
            $.ajax({
                type: "POST",
                url: '<?php echo base_url();?>performainvoice/save_dispatch/',
                data: $("#pi_history_modal_form").serializeArray(), 
                dataType: "html",
                success: function(data){
                    $(".loader").hide();  
                    if(data)
            ...
            var $flatpickr;
        $(document).on("click", ".pi_history", function(event){ 
            var booking_id  = $(this).attr('rel'); 
            $.ajax({
                type: "POST",
                url: '<?php echo base_url();?>performainvoice/pi_history/',
                data: { 'booking_id' :booking_id }, 
                dataType: "html",
                success: function(data){  
                    $(".loader").hide(); 
                    $('.pi_history_list').html(data);
                    $('#pi_history_modal').modal('show');
                    $(".dispatch_date").flatpickr({  
                        dateFormat: "d-m-Y", 
                    }); 
                }
            });
        ...
        $(document).on("click", ".some_button_id", function(event){ 
                alert("Sdasdsa");
                alert($flatpickr);
               $flatpickr.clear();
            });

        $(document).on("keyup", ".freight_input", function(event){             
        ...
        $(document).on("click", ".company_selection", function(event){ 
            event.preventDefault();
            $(".loader").show();
            var confirm_pi = true;
            if(invoice_weight<9)
            ...
            if(confirm_pi)
            {
                $.ajax({
                    type: "POST",
                    url: '<?php echo base_url();?>performainvoice/skulist/',
                    data: $("#pi_form").serializeArray(), 
                    dataType: "html",
                    success: function(data){ 
                        $(".loader").hide();
                        $('.skulist_companyselecetion').html(data);
                        $('#compnayselecteonmodal').modal('show');
                    }
                });
            }
        });
        ...
        $(document).on("click", ".preview_without_frieght", function(event){ 
            event.preventDefault();
            $(".loader").show();
            $.ajax({
                type: "POST",
                url: '<?php echo base_url();?>performainvoice/savecompnay/',
                data: $("#companyselecetion_form").serializeArray(), 
                dataType: "html",
                success: function(data){    
                        $.ajax({
                            type: "POST",
                            url: '<?php echo base_url();?>performainvoice/getcompnaylist/',
                            data: $("#pi_form").serializeArray(), 
                            dataType: "html",
                            success: function(data){   
                                //alert(data);
                                $(".loader").hide();
                                $('.freightlist').html(data);
        ...
        $(document).on("click", ".preview", function(event){ 
            event.preventDefault();
                //var formdata = $("#pi_form").serializeArray();
                //var freightdata = $("#Freight_form").serializeArray();
                var dataString = $("#pi_form, #Freight_form").serialize();
                //formdata.append('freight', freightdata);
                //formdata.push(freightdata);
                //var freight = JSON.stringify(freightdata);
        ...
        $(document).on("click", ".pi_invoice_download", function(event){ 
            event.preventDefault();
                //var formdata = $("#pi_form").serializeArray();
                //var freightdata = $("#Freight_form").serializeArray();
                var dataString = $("#pi_form, #Freight_form").serialize();
                ...
                $.ajax({
                    type: "POST",
                    url: '<?php echo base_url();?>performainvoice/download/',
                    data: dataString, 
                    dataType: "html",
                    success: function(data){ 
                        $(".loader").hide();
        ...
        $(document).on("click", ".pi_invoice", function(event){ 
            event.preventDefault();
            $('.pi_invoice').attr('disabled', 'disabled');
			$('.pi_invoice').prop("disabled",true);
                //var formdata = $("#pi_form").serializeArray();
                //var freightdata = $("#Freight_form").serializeArray();
                var dataString = $("#pi_form, #Freight_form").serialize();
                //formdata.append('freight', freightdata);
        ...
        $(document).on("click", ".pi_invoice11", function(event){ 
            event.preventDefault();
            $(".loader").show();
            $.ajax({
                type: "POST",
...
<div id="pi_history_modal" class="modal fade" role="dialog"> 
  <div class="modal-dialog">
    <!-- Modal content-->
    <div class="modal-content"> 
        <form method="POST" action="" id="pi_history_modal_form">
            <div class="modal-header">
                <h4>PI History</h4>
                <!--<button type="button" class="close" data-dismiss="modal">&times;</button>                 -->
            </div>
            <div class="modal-body pi_history_list"> 
            </div>
            <div class="modal-footer">  
                <input type="submit" class="btn btn-default" name="SaveDispatch" value="Save Dispatch">
...
<div id="deviation_modal" class="modal fade" role="dialog"> 
  <div class="modal-dialog">
    <!-- Modal content-->
    <div class="modal-content"> 
        <form method="POST" action="" id="pi_deviation_modal_form">
            <div class="modal-header">
            ...
            <div class="modal-footer">  
                <input type="submit" class="btn btn-default" name="Savedeviation" value="Save Ddeviation">
                <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
            </div> 
        </form>
    </div>
...
<div id="DispacthSuccess" class="modal fade" role="dialog"> 
  <div class="modal-dialog">
    <!-- Modal content-->
    <div class="modal-content"> 
            <div class="modal-header">
            ...
            <div class="modal-body DispacthSuccess_message">
...
<div id="compnayselecteonmodal" class="modal fade" role="dialog"> 
  <div class="modal-dialog">
    <!-- Modal content-->
    <div class="modal-content"> 
        <form method="POST" action="" id="companyselecetion_form">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal">&times;</button>                 
            </div>
            <div class="modal-body skulist_companyselecetion"> 
            </div>
            <div class="modal-footer"> 
                <button type="button" class="btn btn-default add_frieght" data-dismiss="modal">Add Freight</button>
                <button type="button" class="btn btn-default preview_without_frieght" data-dismiss="modal">Preview</button>
                <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
            </div> 
        </form>
    </div>
  ...
  <div class="modal-dialog">
    ...
    <div class="modal-content">
        <form method="POST" id="">
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-default pi_invoice_download" data-dismiss="modal">Download PI for verification</button> 
                <button type="button" class="btn btn-default pi_invoice" data-dismiss="modal">Generate & Send PI on my whatsapp</button>
                <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
            </div>
        </form>
    </div>
...
$(document).ready(function(){
    ...
    $(document).ready(function(){ 
        ...
        $(document).on("click", ".deviation", function(event){ 
            ...
            $.ajax({
                type: "POST",
                url: '<?php echo base_url();?>performainvoice/piskulist/',
                data: { 'pi_id' :pi_id }, 
                dataType: "html",
                success: function(data){  
                    $(".loader").hide(); 
  ...
  <div class="modal-dialog">
    ...
    <div class="modal-content">
        <form method="POST" id="">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal">&times;</button> 
                <h4 class="modal-title">Notify dispatch confirmation on whatsapp</h4>
            </div>
            <div class="modal-body"> 
