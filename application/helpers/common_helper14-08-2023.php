...
LEFT JOIN `brands` ON `brands`.`id`=`booking_booking`.`brand_id` 
LEFT JOIN `category` ON `category`.`id`=`booking_booking`.`category_id` 
LEFT JOIN `admin` ON `admin`.`id`=`booking_booking`.`admin_id` where ( pi_history.dispatch_date IS NOT NULL and pi_history.dispatch_date <> "" ) and booking_booking.status <> 3 and pi_history.status=0 and vendors.state_id='.$state_id.' and ( pi_history.dispatch_date like  "%'.$dispetched_month.'%" ) '.$condition_emp.' GROUP by booking_booking.category_id order by category.category_name asc'; 
