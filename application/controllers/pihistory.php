...
class Pihistory extends CI_Controller {
    ...
    public function removepi(){ 
        ...
        if($pi_id)
        {
            ...
            $updated = $this->booking_model->UpdatePiHistory($updatedata,$condition);
            if($updated)
            {
                $updatedata_sku = array('pi_id' => 0);
                $remark = "PI -".$pi_id." Removed without unlock";
                if($unlock)
                {
                    $remark = "PI -".$pi_id." Removed with unlock";
                    $updatedata_sku['is_lock'] = 0;
                }
                $condition_sku = array('pi_id' => $pi_id);
                $this->booking_model->UpdateBookingSku($updatedata_sku,$condition_sku);

                $condition_sku = array('pi_number' => $pi_id);
                $this->booking_model->pi_sku_history_bookingremove($condition_sku);



                if($unlock && $booking_id)
                {
                    //$this->booking_model->updatemultiplebooking($booking_id);
                }
                ...
                if($booking_ids)
                {
                    foreach ($booking_ids as $key => $value) {
                        ...
                        if($unlock && $value)
                        {
                            if($this->booking_model->checkbookingstatus($value))
                            {
                                $cond = array('id' => $value);
                                $updatedata = array('is_lock' => 2);
                                $this->booking_model->UpdateBooking($updatedata, $cond); 
                            }
                            else
                            {
                                $cond = array('id' => $value);
                                $updatedata = array('is_lock' => 0);
                                $this->booking_model->UpdateBooking($updatedata, $cond); 
                            }                            
                        }
                        $remarkdata = array('booking_id' => trim($value),'remark' => $remark,'remark_type'=> 'PI Removed','updated_by' => $admin_id);
    ...
    public function index(){    
        ...
        $data["links"] = '';
        if(!empty($_POST) || isset($_SESSION['search__pihistory_data']))
        //if(!empty($_POST))
        {
            if(isset($_POST) && !empty($_POST)) 
                $_SESSION['search__pihistory_data'] = $_POST;
            else
                $_POST = $_SESSION['search__pihistory_data']; 
            ...
            $total_rows =  $this->pi_model->GetPIHistoryCount($condition);
            ...
            //echo $booking_status;
            $data['pis'] = $this->pi_model->GetPIHistory($condition,$limit,$page);
    ...
    public function report_pdf(){    
        ...
        //echo "<pre>"; print_r($data);  print_r($config); die;
        //echo $booking_status;
        $pis = $this->pi_model->GetPIHistory($condition,$limit,$page);
        ...
        if($pis)
        {
            ...
            foreach ($pis as $key => $value) {
                ...
                $html_print .= '<td style="text-align:center; border-right:1px solid #000; border-bottom:1px solid #000;width:100px">'.$value['company_name'].'</td>';
            ...
            $html_print .= '</table>';
            $header = '
                <h3 style="text-align:center;">Performa Invoice Report '.$_REQUEST['from'].' to '.$_REQUEST['to'].'</h3>';
            ...
            $f_name = 'Performa Invoice Report '.$_REQUEST['from'].' to '.$_REQUEST['to'].'.pdf'; 
