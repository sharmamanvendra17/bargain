...
class Excelnew extends CI_Controller {
    ...
    public function index()
    {
        ...
        $objPHPExcel->getActiveSheet()->SetCellValue('J10', 'Rate(Without insurance)'); 
        $objPHPExcel->getActiveSheet()->SetCellValue('K10', 'Rate '); 
        $objPHPExcel->getActiveSheet()->SetCellValue('L10', 'FOR '); 
        ...
        if($bookings)
        {
            ...
            foreach ($bookings as $key => $booking) {
                ...
                $objPHPExcel->getActiveSheet()->SetCellValue('L'.$serial, $insurance1); 
                $originalDate = $booking['created_at'];
                $newDate = date("d-m-Y", strtotime($originalDate));
                $objPHPExcel->getActiveSheet()->SetCellValue('N'.$serial, $newDate); 
                $for = $rate_without_insurance+$booking['for_total'];
                $objPHPExcel->getActiveSheet()->SetCellValue('L'.$serial, $for);
                ...
                if(isset($product_reports[$booking['brand_name']][$booking['category_name']][$booking['product_name']]))
                {
                    ...
                    $objPHPExcel->getActiveSheet()->SetCellValue('L'.$serial, $rate); 
        ...
        $objPHPExcel->getActiveSheet()->SetCellValue('L'.$serial, round($total_amt, 2)); 
