...
class Excelnew extends CI_Controller {
    ...
    public function index()
    {
        ...
        //$objPHPExcel->getActiveSheet()->SetCellValue('I10', 'Rate(Without insurance)'); 
        $objPHPExcel->getActiveSheet()->SetCellValue('J10', 'Rate '); 
        $objPHPExcel->getActiveSheet()->SetCellValue('K10', 'Total'); 
        $objPHPExcel->getActiveSheet()->SetCellValue('L10', 'Date'); 



        $objPHPExcel->getActiveSheet()->getStyle('A10:L10')->applyFromArray($styleArray1);
        ...
        if($bookings)
        {
            ...
            foreach ($bookings as $key => $booking) {
                ...
                $objPHPExcel->getActiveSheet()->SetCellValue('C'.$serial, 'SHAIL/'.$booking['id']);
               ...
               // $objPHPExcel->getActiveSheet()->SetCellValue('I'.$serial, $booking['rate']); 
                $total_qty = $total_qty+$booking['quantity'];


                $insurance1 = round($insurance,2);
                $objPHPExcel->getActiveSheet()->SetCellValue('J'.$serial, $insurance1); 
                $originalDate = $booking['created_at'];
                $newDate = date("d-m-Y", strtotime($originalDate));
                $objPHPExcel->getActiveSheet()->SetCellValue('L'.$serial, $newDate); 
                
                if(isset($product_reports[$booking['brand_name']][$booking['category_name']][$booking['product_name']]))
                {
                    
                    $rate = $booking['quantity']*$insurance;
                    $objPHPExcel->getActiveSheet()->SetCellValue('K'.$serial, $rate); 
        ...
        $objPHPExcel->getActiveSheet()->SetCellValue('K'.$serial, round($total_amt, 2)); 
        ...
        $objPHPExcel->getActiveSheet()->getStyle('B10:L'.$serial)->applyFromArray($styleArray);
