...
class Performance extends CI_Controller {
    ...
    public function index_old(){   
        ...
        if(isset($_POST) && !empty($_POST))
        {
            ...
            if($states)
            {
                ...
                foreach ($states as $key => $value) {
                    ...
                    $response[$key]['makers'] = $this->admin_model->GetBookingsummary( $value['id'],$postdata); 
                    $condition = array("state_id" => $value['id'],'role' => 6);
                    $response[$key]['secondry_makers'] = $this->admin_model->GetSecondaryMakers2($value['id'],$postdata_secondary); 
                }
            }
        }
        else
        ...
        $this->load->view('performance1',$data);
    } 
    public function per(){  
        $this->dynamic->alreadynotLogIn(); 
        $data['title'] = "Sales Report";
        ...
        if(isset($_POST) && !empty($_POST))
        {
            if($states)
            {
                ...
                foreach ($states as $key => $value) {
                    ...
                    $condition = array("state_id" => $value['id']);
                    $response[$key]['state'] =$value;
                    $response[$key]['makers'] = $this->admin_model->GetAllMaker2( $value['id'],$postdata); 
                    $condition = array("state_id" => $value['id'],'role' => 6);
                    $response[$key]['secondry_makers'] = $this->admin_model->GetSecondaryMakers2($value['id'],$postdata_secondary); 
                }
            }
    ...
    public function index(){   
        ...
        $this->load->view('performance2',$data);
    }
