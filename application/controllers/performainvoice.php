...
class Performainvoice extends CI_Controller {
                ...
                $broker_name = ($booking_info['broker_name']) ? $booking_info['broker_name'] : 'Direct';
                  ...
                  $html_response = '<table width="800" border="0"  cellpadding="0" cellspacing="0" style="background:#ffffff; border-collapse:collapse;border:none; border-top:1px solid #000000;  border-bottom:1px solid #000000; text-align:center; margin:0px 20px; font-size:12px;  padding:0px;">
                                        ...
                                        foreach ($skus as $key => $value) { 
                                            ...
                                            if($value['fixed_rate_flag']==0)
                                            {
                                                if($value['packaging_type']!=1)
                                                {
                                                    if(($booking_info['state_id']==4 || $booking_info['state_id']==22 || $booking_info['state_id']== 23 || $booking_info['state_id']==24|| $booking_info['state_id']==25|| $booking_info['state_id']==30|| $booking_info['state_id']==33 || $booking_info['state_id']==3) && $value['base_rate']==0) 
                                                    {
                                                        $nort_east_rate_tin = 10;
                                                        if(strpos($value['name'], '15')===0)
                                                            $nort_east_rate_tin = 0;
                                                        $packing_rate_ltr = ($value['booking_rate']-($value['base_empty_tin_rates']+$nort_east_rate_tin))/$base_weight;
                                                    }
                                                    else
                                                    {
                                                        $packing_rate_ltr = ($value['booking_rate']-$value['base_empty_tin_rates'])/$base_weight;
                                                    }
                                                ...
                                                { 
                                                    ...
                                                    {
														$nort_east_rate_tin = 10;
														$packing_rate_ltr = (($value['booking_rate']-($value['base_empty_tin_rates']+$nort_east_rate_tin))/$base_weight)*$l_to_kg_rate;
													}else{
													$packing_rate_ltr = (($value['booking_rate']-($value['base_empty_tin_rates']+$nort_east_rate_tin))/$base_weight)*$l_to_kg_rate;
                                            ...
                                            {
                                                $sku_rate = round(($value['packing_items_qty']*$value['fixed_rate']),2);
                                            }
                ...
                $party_id = $booking_info['party_id'];
                  ...
                  $html_response = '<table width="800" border="0"  cellpadding="0" cellspacing="0" style="background:#ffffff; border-collapse:collapse;border:none; border-top:1px solid #000000;  border-bottom:1px solid #000000; text-align:center; margin:0px 20px; font-size:12px;  padding:0px;">
                                        ...
                                        foreach ($skus as $key => $value) { 
                                            ...
                                            $total_invoice_amount = $total_invoice_amount+$sku_total_with_gst;
                                            $total_invoice_weight = $total_invoice_weight+$value['weight'];
                                            $total_invoice_weight_net = $total_invoice_weight_net+$value['net_weight'];
                                            $total_invoice_qty = $total_invoice_qty+$value['quantity'];
                                            $sno++;
                    ...
                    $pi_invoice['total_net_weight_pi'] =number_format($total_invoice_weight_net,2);  
                    ...
                    $footerHtml = '<table  style="width:100%; ">
                            ...
                            if($response==21)
                            {
                                ...
                                $remark = "Order from ".$booking_info['party_name']." to supply goods from ".$sendor_party_name." generated by ".$admin_name.". Details attached.";
                                $mobile_number_array =  explode(',', $admin_mobile);
                                foreach ($mobile_number_array as $key => $value) {
                                    $whatsapp_log_data = array('mobile_number' => $value,'message' => $remark,'sender_number'=> $sender_number,'file'=> $pdf_file_name,'receiving_time'=> date('Y-m-d H:i:s'));
                ...
                $broker_name = ($booking_info['broker_name']) ? $booking_info['broker_name'] : 'Direct';
                  ...
                  $html_response = '<table width="800" border="0"  cellpadding="0" cellspacing="0" style="background:#ffffff; border-collapse:collapse;border:none; border-top:1px solid #000000;  border-bottom:1px solid #000000; text-align:center; margin:0px 20px; font-size:12px;  padding:0px;">
                                        ...
                                        foreach ($skus as $key => $value) { 
                                            ...
                                            $total_invoice_amount = $total_invoice_amount+$sku_total_with_gst;
                                            $total_invoice_weight = $total_invoice_weight+$value['weight'];
                                            $total_invoice_weight_net = $total_invoice_weight_net+$value['weight'];
                                            $total_invoice_qty = $total_invoice_qty+$value['quantity'];
                                            $sno++;
    ...
    function pi_history()
    {
        ...
        if($results)
        {
            ...
            $html_response .= "<thead><tr ><th rowspan='2' style='vertical-align: middle;'>S.No.</th><th rowspan='2' style='vertical-align: middle;'>Company Name</th><th rowspan='2' style='vertical-align: middle;'>Weight</th><th rowspan='2' style='vertical-align: middle;'>Bargain No.</th><th rowspan='2' style='vertical-align: middle;'>PI</th><th colspan='2' style='text-align:center'>Dispatch Via</th><th rowspan='2' style='vertical-align: middle;'></th><th rowspan='2' style='vertical-align: middle; border-left: 1px solid #ddd;'>Deviation</th></tr><tr><th style='text-align: center'>Truck No.</th><th  style='text-align: center; border-right: 1px solid #ddd'>Date</th></tr></thead>
        ...
        if($dispatch)
        {
            ...
            foreach ($dispatch as $key => $value) {
                //echo "<pre>"; print_r($value); 
                 //echo "<pre>"; print_r($whatsapp[$key]['dispatched']);  die;
                ...
                if($booking_ids1)
                {
                    foreach ($booking_ids1 as $booking_idskey => $booking_ids_value1) {
                        $booking_id_array1 = explode(',', $booking_ids_value1);
                        if($booking_id_array1)
                        {
                            foreach ($booking_id_array1 as $booking_id_array1key => $booking_ids_value2) {         

                                if($whatsapp[$key]['dispatched']==0)
                                {                    
    ...
    public function whatsappusers()
    {
        ...
        $booking_ids = $_POST['booking_ids'];
        $pi_id = $_POST['pi_id'];
        $bargain_ids = $_POST['bargain_ids'];

        $pi_condition = array('pi_history.id' => $pi_id);
        $pi_info = $this->pi_model->GetPiInfo($pi_condition);
        $truck_number = $pi_info['truck_number'];
        $dispatch_date = $pi_info['dispatch_date'];
        if($truck_number)
        {
            $users = $this->booking_model->whatsappusers($booking_ids);
            $response= ' 
                <input type="hidden" id="notify_pi_id" name="notify_pi_id" value="'.$pi_id.'">
                <input type="hidden" id="notify_bargain_ids" name="notify_bargain_ids" value="'.$bargain_ids.'">
            ...
            if($users)
            {
                ...
                foreach ($users as $key => $value) {
                    $response .=
                            "<tr>
                                <td>".$i."</td>
                                <td><input name='users' type='checkbox' class='user_select' value='".$key."' checked ></td>
                                <td>".$value."</td>
                                <td>".$key."</td>
                            </tr>";
                    $i++;
    ...
    public function whatsapppdf(){
        ...
        if($users)
        {
            ...
            $message_params = urlencode("#DATA/".$bargain_ids."~".$truck_number."~".$dispatch_date); 
            $curl_watsappapi = curl_init(); 
            curl_setopt_array($curl_watsappapi, array( 
            CURLOPT_URL => 'https://dlr.dil.in/notify.jsp?TYPE=WAB&F=918764216255&T='.$mobile_number.'&TID=9791655&P='.$message_params,
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_ENCODING => '',
            CURLOPT_MAXREDIRS => 10,
            CURLOPT_TIMEOUT => 0,
            ...
            {
                $sender_number = '918764216255';
                $remark = "Your bargain no.#DATA/".$bargain_ids." has been dispatched via ".$truck_number." on ".$dispatch_date;
                $mobile_number_array =  explode(',', $mobile_number);
                foreach ($mobile_number_array as $key => $value) {
                    $whatsapp_log_data = array('mobile_number' => $value,'message' => $remark,'sender_number'=> $sender_number,'receiving_time'=> date('Y-m-d H:i:s'));
                    $this->booking_model->AddWhatsappLog($whatsapp_log_data);
                }                                
            }
        }
